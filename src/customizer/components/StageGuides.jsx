 ...
 */
function StageGuides({ template, scale, snapGuides = [], showGrid = false }) {
  ...
  if (showGrid) {
    ...
    for (let x = stepMm; x < template.trim.width + template.bleed * 2; x += stepMm) {
      gridLines.push(<line key={`gx${x}`} x1={x * scale} y1={0} x2={x * scale} y2={canvasH} stroke="rgba(23,24,27,0.06)" strokeWidth="1" />);
    }
    for (let y = stepMm; y < template.trim.height + template.bleed * 2; y += stepMm) {
      gridLines.push(<line key={`gy${y}`} x1={0} y1={y * scale} x2={canvasW} y2={y * scale} stroke="rgba(23,24,27,0.06)" strokeWidth="1" />);
    }
  }
