 ...
 *
 * Shared by the in-editor Templates panel and the pre-editor Start screen so
 * a customer sees the same layouts whether they pick one before opening the
 * canvas or after.
 */
export const buildStarters = (template, productName) => {
 ...
 * real content is applied as editable layers on click. Percent-based sizing
 * means one component scales cleanly from a 256px panel to a start-screen card.
 */
export function StarterThumb({ id, aspect }) {
  return (
    <div
...
export const starterAspect = (template) =>
  template?.trim ? `${template.trim.width} / ${template.trim.height}` : "7 / 4";
